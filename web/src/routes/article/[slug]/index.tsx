...
export default component$(() => {
  ...
  if (store.notFound) {
    return <div>{t('articles.notFound')}</div>;
  }
      ...
      onResolved={(content) => (
        <article class={[
          ...
          <div role="alert" class="alert alert-warning opacity-75 mb-4">
            ...
            <span><b>{t('articles.warning')}</b>: {article.warningMessage[locale.value] || article.warningMessage.en}</span>
          </div>
          )}
...
export const head: DocumentHead = {
  title: "Articles | Digital Defense",
  meta: [
    {
      name: "description",
      content: "Articles and guides on digital security and privacy.",
    },
  ],
};
